...
class ApiBirds {
    ...
    suspend fun fetchBirdImages(): List<BirdImage> {
        ...
        //val json = Json.decodeFromString<JsonElement>(content)
        //    .jsonObject["payload"]?.jsonObject?.get("blob")?.jsonObject?.get("rawLines")?.jsonArray?.get(0)
        //    .toString()

        return Json.decodeFromString<List<BirdImage>>(
            """
                    [
                    {
                "category": "PIGEON",
